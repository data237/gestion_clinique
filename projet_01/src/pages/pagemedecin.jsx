...
import imgtest from '../assets/IconRendezvous.png' // Utilisons l'icône rendez-vous temporairement
...
function PageMedecin(){
    ...
    <PageStyle>
        <Barrelatteral> 
            ...
            <Link to="/medecin/calendrier" className={contenuActif === 'calendrier' ? 'eltmenu' : 'lienadmin'} onClick={() => {changerContenu('calendrier')}}><Eltmenu nommenu='Calendrier' img={imgcalendrier} className={contenuActif === 'patient' ? 'eltmenu' : ''} onClick={() => {changerContenu('calendrier')}}/></Link>
            <Link to="/medecin/test-prescription" className={contenuActif === 'test-prescription' ? 'eltmenu' : 'lienadmin'} onClick={() => {changerContenu('test-prescription')}}><Eltmenu nommenu='Test PDF' img={imgtest} /></Link>
